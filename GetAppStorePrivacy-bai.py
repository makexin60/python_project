...
def handel_all_sub_privacy():

    with open(r"data_csv\policy_todo\appstore_appInfo_todo.csv", mode="r", encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            if original_url:
                try:
                    ...
                    if privacy_data != "":
                        GetCommon.save_new_txt_file(f"Policy_appstore/b/main/{app_id}_policy", privacy_data)
                        logger.warning(json.dumps(json_url, ensure_ascii=False))
                            ...
                            try:
                                ...
                                if len(visible_privacy_links) != 0:
                                    ...
                                    if len(filtered_urls) != 0:
                                        ...
                                        # }
                                        # sub_privacy_data, results = GetCommon.wait_extra_privacy_only_sub(json_url, filtered_urls,driver,logger)
                                        GetCommon.save_new_txt_file(f"Policy_appstore/b/sub/{app_id}_subPolicy", ','.join(filtered_urls))
                                        logger.warning(json.dumps(json_url, ensure_ascii=False))
                                else:
                                    print(" no find extra <a>")
...
if __name__ == "__main__":
    ...
    logger = GetCommon.getLogger("AppStorePrivacyLoop-test-bai")
